...
func (h *History) buildVi(ctx context.Context, item *filesItem, p *background.Progress) (err error) {
	...
	return buildVi(ctx, item, iiItem, idxPath, h.tmpdir, count, p, h.compressVals)
}
...
func buildVi(ctx context.Context, historyItem, iiItem *filesItem, historyIdxPath, tmpdir string, count int, p *background.Progress, compressVals bool) error {
...
func (hi *HistoryChangesIterDBDup) advance2() (err error) {
	var k, v []byte
	if hi.txNum2kCursor == nil {
		if hi.valsCursor, err = hi.roTx.CursorDupSort(hi.valsTable); err != nil {
			return err
		...
		k, v, err = hi.txNum2kCursor.Seek(hi.startTxKey[:])
	} else {
		k, v, err = hi.txNum2kCursor.Next()
	}
	if err != nil {
		return err
	}
	...
	vv, err := hi.valsCursor.SeekBothRange(v, k)
	if err != nil {
		return err
	}
	if binary.BigEndian.Uint64(vv) != txNum {
